...
get_attachment_id() {
    /usr/bin/aws ec2 describe-network-interfaces --network-interface-id ${OCF_RESKEY_eni_id} --query NetworkInterfaces[0].Attachment.AttachmentId --output text
}
...
attach_eni() {
    ...
    while [ "${attachment_id}" = "None" ]; do
        /usr/bin/aws ec2 attach-network-interface --network-interface-id ${OCF_RESKEY_eni_id} --instance-id ${instance_id} --device-index ${OCF_RESKEY_device_index}
        sleep 5
        attachment_id=$(get_attachment_id)
    done
}
...
get_route_table_id() {
  private_ip=$(/sbin/ip addr show dev ${OCF_RESKEY_device_name} | awk '/inet /{split($2,a,"/"); print a[1]}')
  /sbin/ip rule | grep ${private_ip} | awk '{print $NF}'
}
...
eni_start() {
    ...
    /sbin/ip route add default via ${OCF_RESKEY_gateway} dev ${OCF_RESKEY_device_name} table ${route_table_id}
...
eni_monitor() {
    ...
    mon=$(/usr/bin/aws ec2 describe-network-interfaces --network-interface-ids ${OCF_RESKEY_eni_id} --filter "Name=attachment.instance-id, Values=${instance_id}" --output json | grep ${instance_id} | wc -l)
    if [ $mon -ne 0  ]; then
        route_table_id=$(get_route_table_id)
        #default_route_count=$(/sbin/ip route show table ${route_table_id} | /usr/bin/grep -c "^default.*${OCF_RESKEY_gateway}.*${OCF_RESKEY_device_name}")
        default_route_count=$(/sbin/ip route show table ${route_table_id} | /usr/bin/grep -c "^default")
